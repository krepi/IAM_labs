...
6.  **Dziennik Synchronizacji (SYNC_LOG.md):** Kontekst repozytorium jest współdzielony (Mac / PC). Na komendę dodania wpisu do dziennika, Agent ma obowiązek uzupełnić `SYNC_LOG.md`. **KRYTYCZNE ZASADY:** Nowy wpis musi znajdować się ZAWSZE NA SAMEJ GÓRZE (kolejność odwrotnie chronologiczna). Nagłówek musi bezwzględnie wskazywać urządzenie, na którym toczy się sesja, w formacie: `## [YYYY-MM-DD] - Mac Session: [Tytuł]` lub `## [YYYY-MM-DD] - PC Session: [Tytuł]`.
